...
ID** and `2` is **Tenant ID**.

To make it work with Cube.js, first we need to pass the `appId` and `tenantId`
as context to every query. We should include them into our token generation
code.
...
const cubejsToken = jwt.sign({ appId: '1', tenantId: '2' }, CUBE_API_SECRET, {
  expiresIn: '30d',
});
```

Now, we can access them through the [`securityContext`][ref-config-security-ctx]
property inside the context object. Let's first use
...
module.exports = {
  contextToAppId: ({ securityContext }) =>
    `CUBEJS_APP_${securityContext.appId}_${securityContext.tenantId}`,
};
```

Next, we can use [`driverFactory`][ref-config-driverfactory] to dynamically
select database, based on `appId` and `tenantId`.
...
module.exports = {
  ...
  driverFactory: ({ securityContext }) =>
    new PostgresDriver({
      database: `my_app_${securityContext.appId}_${securityContext.tenantId}`,
    }),
};
```
